...
func (s *Server) setupRoutes() {
	...
	// 静态文件服务 (嵌入的前端)
	if s.staticFS != nil {
		// Vite build 默认资源路径（/assets/... 与 /vite.svg）
		s.engine.GET("/assets/*filepath", func(c *gin.Context) {
			...
			full := path.Join("assets", p)
			data, err := fs.ReadFile(s.staticFS, full)
			if err != nil {
				c.String(http.StatusNotFound, "not found")
				return
			}
		...
		s.engine.GET("/vite.svg", func(c *gin.Context) {
			data, err := fs.ReadFile(s.staticFS, "vite.svg")
			if err != nil {
				c.String(http.StatusNotFound, "vite.svg not found")
				return
			}
	...
	// Local analysis report (generated by `stockctl -analyze`).
	// Serve local `runtime/analysis/` at /analysis (avoid gin.StaticFS wildcard conflicts).
	analysisRoot := filepath.Clean("runtime/analysis")
	s.engine.GET("/analysis", func(c *gin.Context) {
		c.Redirect(http.StatusTemporaryRedirect, "/analysis/")
	})
	s.engine.GET("/analysis/*filepath", func(c *gin.Context) {
		raw := c.Param("filepath") // includes leading '/' (or empty)
		fp := strings.TrimPrefix(raw, "/")
		if fp == "" || strings.HasSuffix(raw, "/") {
			fp = "index.html"
		}
		...
		full := filepath.Join(analysisRoot, clean)
		if filepath.Clean(full) != analysisRoot && !strings.HasPrefix(filepath.Clean(full)+string(filepath.Separator), analysisRoot+string(filepath.Separator)) {
			c.String(http.StatusNotFound, "not found")
			return
		}
		...
		if _, err := os.Stat(full); err != nil {
			if fp == "index.html" {
				c.String(http.StatusNotFound, "analysis report not found: runtime/analysis/index.html\nRun: make analyze ANALYZE_DAYS=365 (or set ANALYZE_OUT_DIR=runtime/analysis)\n")
				return
			}
			c.String(http.StatusNotFound, "not found")
			return
